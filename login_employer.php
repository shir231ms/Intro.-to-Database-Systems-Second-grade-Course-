<?php 
	...
	if( !isset( $_SESSION['Account'])  )
			...
			if($_GET["search"]=="Search"){
				...
				while($data = $sth222->fetchObject()){
					...
					<tr>
						...
						<th><?php
							$sql0="SELECT * FROM `employer`"."WHERE `id`=?";
							$sth0=$db->prepare($sql0);
							$sth0->execute(array($data->employer_id));
							$result0=$sth0->fetchObject();
							if($result0->account==$_SESSION["Account"]){?>
								<form action="edit_nj.php"method="POST">
									<input type="hidden" name="revise" value="<?php echo $data->id?>">
									<button type="submit"name="submit"value="yes">Edit</button>
								</form>
								<?php echo " ";?>
								<form action="delete.php"method="POST">
									<input type="hidden" name="revise" value="<?php echo $data->id?>">
									<button type="submit"name="submit" value="no">Delete</button>
								</form><?php
							}?>
						</th>
					</tr>
				...
				if($_POST["sort"]=="Asc"){
					$result3=$db->query("SELECT * FROM `result` ORDER BY `salary` ASC, `id` ASC");
				}
				else if($_POST["sort"]=="Des") {
					$result3=$db->query("SELECT * FROM `result` ORDER BY `salary` DESC, `id` ASC");
				}
				else {
		...
		<form method="POST">
			<input type="button" value="Add a New Job" onclick="location.href='add_nj.php'"/>
			<input type="button" value="Job Seeker List" onclick="location.href='list_js.php'"/>
			<input type="button"value="Who Applies Your Job" onclick="location.href='app_list.php'"/>
		</form><?php
	}
?>
